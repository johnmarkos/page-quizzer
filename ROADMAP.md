...
- [x] **Q7: Question quality filter** — Add a lightweight post-generation pass that rejects or regenerates questions with weak distractors, trivial fact recall, or obvious answer-pattern giveaways. Test: pure heuristics where feasible, manual review on sample pages.
...
- [x] **D8: Export quiz as OpenQuizzer JSON** — Add an "Export as OpenQuizzer" option that downloads the current quiz in OpenQuizzer's native chapter JSON format, reusing the existing export data flow and covering the conversion with panel-side unit tests.
